...
- [x] 📐 Создание схемы БД (Database First подход)
  - [x] Таблица `Users` (Id, Username, Email, PasswordHash, Role, CreatedAt, UpdatedAt)
...
- [x]  Ролевая модель (Admin/User)
  - [x] Колонка `Role` в таблице Users
  - [x] Role claim в JWT токене
...
- [x] ➕ POST `/api/Admin/users` — создание новых пользователей
...
- [x] 📄 Профессиональный `.http` файл в репозитории
  ...
  - [x] 10 тестовых сценариев (Auth, Logout, Admin, Token Cleanup)
  - [x] Разделение на логические секции с комментариями
- [x] ✅ Тестирование через Visual Studio REST Client

### 📚 Документация
- [x]  Полный README.md (15 шагов)
...
- [ ]  IHostedService для автоматической очистки токенов
  ...
  - [ ] Удаление токенов с ExpiresAt < NOW()
  - [ ] Логирование количества удалённых записей

### 📊 Расширенное логирование
- [ ] ️ Чистый SQL лог в консоли
  - [ ] Вывод реальных значений параметров вместо `?`
  - [ ] Настройка через RelationalEventId
...
- **Ветка разработки:** `RefreshToken` (текущая), `master` (стабильная)
...
*Последнее обновление: 08.09.2026 | Автор: TaskFlow Team*